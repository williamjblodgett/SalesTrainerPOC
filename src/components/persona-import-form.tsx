...
type SourceDraft = { sourceId: string; title: string; content: string; status: "empty" | "parsing" | "ready" | "error"; note?: string; storagePath?: string; provider?: string; scannerStatus?: string; piiFindings?: Array<{ kind: string; count: number }>; originalFilename?: string; originalMimeType?: string; originalSizeBytes?: number };
type ClaimReview = { disposition: "accepted" | "edited" | "rejected"; replacementClaim?: string };
...
export function PersonaImportForm({ industries }: { industries: IndustryOption[] }) {
  ...
  const [retentionMode, setRetentionMode] = useState<"redact_then_delete" | "retain_until_deleted">("retain_until_deleted");
  ...
  async function parseFile(sourceId: string, files: FileList | null) {
    ...
    form.set("retentionMode", retentionMode);
    try {
      ...
      updateSource(sourceId, { title: files[0].name, content: body.text, status: "ready", note: `${body.scan.status} · ${body.redactedCount} sensitive value${body.redactedCount === 1 ? "" : "s"} redacted`, storagePath: body.storagePath, provider: "upload", scannerStatus: body.scan.status, piiFindings: body.piiFindings, originalFilename: body.originalFilename, originalMimeType: body.originalMimeType, originalSizeBytes: body.originalSizeBytes });
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch("/api/personas/extract", {
        ...
        body: JSON.stringify({ industryId, retentionMode, consentAttested: true, transcripts: evidenceInput }),
      });
      const body = await response.json();
  ...
  async function transition(action: "approve" | "publish") {
    ...
    if (personaId && action === "approve") {
      const reviewResponse = await fetch(`/api/personas/${personaId}/claims`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ reviews: draft.evidenceClaims.map((claim) => ({ claimId: claim.id, ...reviews[claim.id] })) }) });
  ...
  if (draft) return <div className="space-y-5">
    ...
    <section className="card"><div className="flex justify-between gap-3"><div><h2 className="text-lg font-semibold">Review every evidence claim</h2><p className="text-sm text-slate-500">{Object.keys(reviews).length} of {draft.evidenceClaims.length} reviewed</p></div><span className="badge">{evidenceInput.length} source{evidenceInput.length === 1 ? "" : "s"}</span></div><div className="mt-4 grid gap-3">{draft.evidenceClaims.map((claim) => <article className="rounded-xl border border-slate-200 p-4" key={claim.id}><div className="flex flex-wrap justify-between gap-3"><strong className="text-sm capitalize">{claim.claimType.replaceAll("_", " ")}</strong><span className="badge">{Math.round(claim.confidence * 100)}% · {claim.origin} · {claim.sourceId.slice(0, 8)} · {claim.turnId}</span></div><p className="mt-2 text-sm">{claim.claim}</p><blockquote className="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">“{claim.excerpt}”</blockquote><div className="mt-3 flex flex-wrap gap-2"><button className={reviews[claim.id]?.disposition === "accepted" ? "button" : "button-secondary"} onClick={() => setReviews((current) => ({ ...current, [claim.id]: { disposition: "accepted" } }))}>Accept {claim.claimType.replaceAll("_", " ")} claim</button><button className={reviews[claim.id]?.disposition === "edited" ? "button" : "button-secondary"} onClick={() => setReviews((current) => ({ ...current, [claim.id]: { disposition: "edited", replacementClaim: current[claim.id]?.replacementClaim ?? claim.claim } }))}>Edit claim</button><button className={reviews[claim.id]?.disposition === "rejected" ? "button" : "button-secondary"} onClick={() => setReviews((current) => ({ ...current, [claim.id]: { disposition: "rejected" } }))}>Reject {claim.claimType.replaceAll("_", " ")} claim</button></div>{reviews[claim.id]?.disposition === "edited" && <label className="mt-3">Manager-approved wording<input value={reviews[claim.id].replacementClaim ?? ""} onChange={(event) => setReviews((current) => ({ ...current, [claim.id]: { disposition: "edited", replacementClaim: event.target.value } }))}/></label>}</article>)}</div></section>
