...
The Supabase workflow authenticates with the Management API, links the explicitly requested project so the CLI resolves its correct region, performs a migration dry run, applies migrations only when requested, and runs every pgTAP isolation suite. The separate auth workflow applies canonical URLs, disables public signup, and installs token-hash recovery/invite templates through the Supabase Management API.
