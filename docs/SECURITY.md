...
Supabase sessions are refreshed at the application proxy and identity is revalidated with `auth.getUser()` before organization context is resolved. Recovery and invitation token hashes are verified server-side; the browser never manually replays access tokens. Password changes require a verified user session plus a short-lived HTTP-only recovery-intent cookie and end with global sign-out. Organization creation, assignment targeting, and asset review are atomic database operations.

Tenant access is resolved from authenticated membership, never a browser-supplied organization ID. Active-organization cookies are accepted only after membership validation. Every tenant table has `organization_id`, RLS is enabled, and server routes repeat authorization checks. Reps cannot directly select private scenario versions, Revenue OS call evidence, graph state, connector state, or peer sessions/transcripts; manager/owner review access is scoped to their organization.

OpenAI, Supabase service, and Cloudmersive keys remain server-only. Practice clients receive only `repVisible`; private BuyerState has no browser-facing RLS policy. Transcript sources, segments, drafts, persona versions, originals, and citations are manager/owner-only. Retained originals use the private `transcript-originals` bucket and customer-selected deletion. Documents are untrusted, validated by type and size, passed through Cloudmersive Advanced in production, and redacted before model input. Scanner outages fail closed. Raw voice audio will not be retained by default.
