...
- `src/app/` is the single canonical Next.js application on Vercel. It owns marketing, Supabase authentication, private product routes, and account recovery.
- GitHub Pages contains only a branded redirect to the canonical origin.
- `public/data/industry-packs.json` is the launch catalog used by the canonical application.

The retired Sites/D1 compatibility runtime is not an approved product surface. No user should need a ChatGPT account to reach Suadence.
...
The private pilot uses Next.js with Supabase Postgres/RLS, private storage for explicitly retained source files, and server-only provider adapters. Durable background jobs and encrypted connector credentials remain required before native connector launch.
...
The canonical application uses Supabase SSR sessions, a proxy-based cookie refresh boundary, server-side organization context, and atomic database functions. Recovery and invitation links are verified server-side with token hashes before a short-lived, HTTP-only recovery session can change a password.
